...
namespace Maritimecode.Services
{
    ...
    {        
        private readonly IRandonNumberRepository _rNR;

        public CalculationServices(NumberContext nDb, IRandonNumberRepository rNR)
        {           
            _rNR = rNR;
        }
        public async Task<bool> CanProcess()
        {
            var randomNumbers = await _rNR.GetRanNumbersAsync();            
            return randomNumbers.Count() > 1  ;
        }

        public async Task ProcessFile(UploadModel vm)
        {
            ...
            if (result == ranEntityList.Count
                && ranEntityList.Count > 1)
            {
        ...
        public async Task<double> StandardDevAsync()
        {            
            ...
            foreach (var item in nums)
            {
                var first = item.NumberValue - mean;
                first *= first;
                d += first;
            }
            var second = d / (nums.Count() - 1); // as its a sample
